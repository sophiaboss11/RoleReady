...
import { HlmIcon } from '@app/ui/icon';
...
@Component({
	...
	hostDirectives: [{ directive: BrnSelectItem, inputs: ['disabled', 'value'] }],
	template: `
		<span class="absolute end-2 flex size-3.5 items-center justify-center">
			@if (this._brnSelectItem.active()) {
...
export class HlmSelectOption {
	protected readonly _brnSelectItem = inject(BrnSelectItem, { host: true });
	constructor() {
		classes(
			() =>
